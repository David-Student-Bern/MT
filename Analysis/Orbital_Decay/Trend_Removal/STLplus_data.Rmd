...
## Read generated Data (parquet)

```{r, echo=FALSE, warning=FALSE, message=FALSE}
library(arrow)

# Path to the parquet file
GFOC_path <- path(repo_root, "Dataset", "Dataset_MSc", "GFOC_RDCDFI.parquet")
...
data <- tryCatch(
  {
    # Use readr::read_csv for efficiency (similar to pandas)
    dat <- read_parquet(GFOC_path, col_select = all_of(columns_needed))
    message(sprintf("Successfully loaded data with shape: %d rows, %d cols",
                    nrow(dat), ncol(dat)))
    dat
  },
...
## Some modifications to the dataset (run only for csv)
...
labels <- c(
  ...
  paste0(format(breaks[5], "%Y-%m-%d"), "_to_", format(breaks[6], "%Y-%m-%d"))
)
...
stl_decompose <- function(period_days, x, time) {
  ...
  # Convert days to half minutes (data sample rate for csv)
  # period <- period_days * 24 * 120
  
  # Convert days to 20 seconds (data sample rate for parquet)
  period <- period_days * 24 * 60 * 3
...
res_D <- stl_decompose(
  period_days = 27.77,
  x = data_groups$`2023-01-01_to_2023-05-01`$orbital_decay,
  time = data_groups$`2023-01-01_to_2023-05-01`$time
)
```
...
data_groups$`2023-05-01_to_2023-10-01`$recreated_wE <- res_D$data$seasonal + res_D$data$trend
data_groups$`2023-05-01_to_2023-10-01`$recreated_f  <- res_fD$data$seasonal + res_fD$data$trend
